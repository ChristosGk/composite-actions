...
1. Edit the file `composite-action-workflow.yml` to replace the following placeholders with the correct values:
    ...
    ```
1. If your username is `name` and the composite repo name is `repo-action`, your `composite-action-workflow.yml` should now include a line as follows:
    ```
    uses: name/repo-action@dev
    ```
